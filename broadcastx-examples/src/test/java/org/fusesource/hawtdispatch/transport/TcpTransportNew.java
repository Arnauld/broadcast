 ...
 */
public class TcpTransportNew extends ServiceBase implements Transport {
    ...
    public void _start(Task onCompleted) {
        System.out.println("TcpTransportNew._start");
        try {
            if (socketState.is(CONNECTING.class)) {
                ...
                this.blockingExecutor.execute(new Runnable() {
                    public void run() {
                        try {
                            System.out.println("TcpTransportNew._start(blockingExecutor#run)");
                            ...
                            System.out.println("TcpTransportNew._start(blockingExecutor#run)... resolving remote...");
                            String host = resolveHostName(remoteLocation.getHost());
                            System.out.println("TcpTransportNew._start(blockingExecutor#run)... resolving remote: " + host);

                            final InetSocketAddress remoteAddress = new InetSocketAddress(host, remoteLocation.getPort());
                            System.out.println("TcpTransportNew._start(blockingExecutor#run)... remoteAddress: " + remoteAddress);

                            System.out.println("TcpTransportNew._start(blockingExecutor#run)... dispatchQueue: " + dispatchQueue);

                            // Done resolving.. switch back to the dispatch queue.
                            dispatchQueue.execute(new Task() {
                                @Override
                                public void run() {
                                    System.out.println("TcpTransportNew._start(blockingExecutor#run(dispatchQueue#run)");
    ...
    private void trace(String message) {
        System.out.println("TcpTransportNew.trace(" + message + ")");
    }

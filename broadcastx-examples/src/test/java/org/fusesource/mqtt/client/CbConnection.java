...
public class CbConnection {
    ...
    void reconnect(final Callback<Transport> onConnect) {
        System.out.println("CbConnection.reconnect...");
    ...
    void createTransport(final Callback<Transport> onConnect) throws Exception {
        ...
        System.out.println("CbConnection.createTransport::" + scheme);

        final Transport transport;
        if( "tcp".equals(scheme) ) {
            transport = new TcpTransportNew();
        ...
        if( transport instanceof TcpTransportNew ) {
            TcpTransportNew tcp = (TcpTransportNew)transport;
        ...
        transport.setTransportListener(new DefaultTransportListener(){
            public void onTransportConnected() {
                System.out.println("CbConnection.createTransport(TransportListener#onTransportConnected)");
                if(disconnected) {
            ...
            private void onFailure(final Throwable error) {
                System.out.println("CbConnection.createTransport(TransportListener#onFailure)");
                error.printStackTrace();
        ...
        transport.start(new Runnable() {
            @Override
            public void run() {
                System.out.println("CbConnection.createTransport(run)");
            }
        });
    }
        ...
        if(mqtt.getKeepAlive()>0) {
            ...
            heartBeatMonitor.setOnKeepAlive(new Task() {
                public void run() {
                    ...
                    if(!disconnected && pingedAt==0 && CbConnection.this.transport.offer(new PINGREQ().encode())) {
                        final long now = System.currentTimeMillis();
                        final long suspends = suspendChanges.get();
                        pingedAt = now;
                        queue.executeAfter(CbConnection.this.mqtt.getKeepAlive(), TimeUnit.SECONDS, new Task() {
                            public void run() {
                                if (now == pingedAt) {
    ...
    public CbConnection refiller(Runnable refiller) {
        queue.assertExecuting();
        this.refiller = refiller;
        return this;
    }

    public CbConnection listener(Listener listener) {
